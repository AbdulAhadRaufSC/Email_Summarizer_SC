...
- **Tesseract OCR engine** — a *system* package (`apt-get install tesseract-ocr`), **not**
...
own `Field(default_factory=...)`, and pydantic-settings' `env_file` loading is *per class* —
...
- **Quality gates** (run locally / should be CI):
  ...
  ```
- **CI/CD: none configured** — no `.github/`, no pipeline files exist. Tests run on-demand
  locally. This is a documented gap.

## 5. Deployment strategy

_Inferred from implementation + `CLAUDE.md`; no Dockerfile or IaC exists in the repo yet._
...
3. **No horizontal-scaling coordination needed** — CAS makes multiple worker instances safe
   ...
   *available for free*; the GPU is what actually gates it.

**Scaling levers:**
- *Horizontal:* run N consumer tasks against the same queue (safe via CAS + visibility
  timeouts). Gains are capped by RunPod capacity.
- *Vertical:* more RunPod workers / a bigger GPU tier; raise `SQS_CONCURRENCY` in step.
- *Future:* the deferred backfill script's own bounded concurrency protects live traffic from
  the 40k backfill without a second queue.

Designed-for-growth choices that don't preclude scale: standard (not FIFO) SQS + DB-level
CAS, stateless per-message processing, versioned self-describing summaries (replayable for a
future vector index).
