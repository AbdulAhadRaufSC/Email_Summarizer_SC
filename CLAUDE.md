...
- **A separate, unresolved real-data finding, not yet fixed**: ticket 239907 (7 emails) has one historical email whose `Email_Metadata` row exists in MySQL but whose body returns `200 []` (empty) from the Email API, consistently and repeatedly — not a transient blip. Under the current design this is treated as `EmailNotYetAvailable` (transient) for *any* referenced email, not just the triggering one, so this ticket will never complete via the CLI (and would only reach DLQ after real SQS retry exhaustion, once SQS exists). This is real evidence for the still-open R6 question from `CONTEXT.txt`: whether a permanently-missing non-triggering email should instead degrade the summary to `PARTIAL` rather than blocking the ticket indefinitely. Needs a product decision, not a code fix.
