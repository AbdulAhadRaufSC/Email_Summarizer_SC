...
- Phase 5 (implementation): in progress, following the module-by-module build order in `implementation_plan.md`. Built and stabilized (unit-tested, `mypy --strict` clean, `ruff` clean):
  ...
  - 173 unit tests passing (`uv run pytest`), `mypy --strict` and `ruff` both clean on `src/`; 8 integration tests for `MySqlSummaryRepository` still unexecuted (see Open Questions — needs Docker).
