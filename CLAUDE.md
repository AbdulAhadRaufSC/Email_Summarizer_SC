...
- Phase 5 (implementation): in progress, following the module-by-module build order in `implementation_plan.md`. Built and stabilized (unit-tested, `mypy --strict` clean, `ruff` clean):
  ...
  - `adapters/llm/runpod_vllm_client.py` — `RunpodVllmClient`, matching the async `/run` + poll `/status` pattern from the user's `runpod_context.py` reference. **Unverified assumption, flagged in the module docstring**: the guided-JSON field is sent as `sampling_params.guided_decoding.json` (current vLLM ≥0.6 convention). Older `worker-vllm` deployments expect a flat `sampling_params.guided_json` instead — there's no reachable RunPod endpoint from this dev sandbox to confirm which this deployment uses. It's isolated to one constant (`_GUIDED_JSON_PARAM_KEY`) to fix once tested against the real endpoint. A RunPod job `FAILED` status is currently treated as retryable (`LlmTransient`, since there's no fallback LLM provider) rather than terminal — a genuinely bad prompt would retry until DLQ rather than fail fast.
  ...
  - 162 unit tests passing (`uv run pytest`), `mypy --strict` and `ruff` both clean on `src/`; 8 integration tests for `MySqlSummaryRepository` still unexecuted (see Open Questions — needs Docker).
- **The full pipeline is implemented end-to-end** (all steps of `implementation_plan.md`), but has never been run against real infra (MySQL, the internal Email API, or RunPod) — only unit-tested with fakes. Run the CLI against staging before trusting it in anger, per the plan's "Manual Verification" section.
- A `runpod_context.py` reference snippet (RunPod async `/run` + poll `/status` pattern, from an unrelated prior project) sits at the repo root — it was the model for `RunpodVllmClient` and is not itself part of the package.
