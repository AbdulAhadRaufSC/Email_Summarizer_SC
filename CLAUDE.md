...
- **SQS live-queue consumer built, 2026-07-21 — `entrypoints/sqs_consumer.py`, `config/settings.py::SqsSettings`.** Thin wrapper around the same `SummarizeTicket` orchestrator the CLI drives, per the "Event flow" / "Queueing" sections above. Key decisions, informed by `SQS_Pipeline_context.js` (a different team's reference Node.js SQS worker checked into this repo's root) and by facts confirmed with the user this same session:
  ...
  - **Not yet run against the real live queue** — per the user's explicit call this session, live verification was deferred rather than done now (contrast with every other module in this project, which has been run against real staging infra before being trusted — see the repeated pattern above). Do that before actually cutting real traffic onto this consumer: send one real `{ticketId, emailMetaId, threadId}` message to `steppingcloud-stage-ticket-ai-summary-queue` and confirm `python -m summarizer.entrypoints.sqs_consumer` picks it up, processes it, and deletes it.
